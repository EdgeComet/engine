...
`
	...
	assert.Len(t, cfg.Dimensions, 2)

	// Verify desktop dimension
	desktop, exists := cfg.Dimensions["desktop"]
	...
	mobile, exists := cfg.Dimensions["mobile"]
	assert.True(t, exists)
	...
	require.NotNil(t, host.Dimensions)
	assert.Len(t, host.Dimensions, 3) // desktop + mobile + bypass (injected)

	// Verify inherited desktop dimension
	inheritedDesktop, exists := host.Dimensions["desktop"]
	assert.True(t, exists)
	...
	inheritedMobile, exists := host.Dimensions["mobile"]
	assert.True(t, exists)
...
`
	...
	require.NotNil(t, host.Dimensions)
	assert.Len(t, host.Dimensions, 2) // custom + bypass (injected)

	// Verify custom dimension (not global)
	custom, exists := host.Dimensions["custom"]
	assert.True(t, exists)
	...
	_, exists = host.Dimensions["desktop"]
...
`
	...
	require.NotNil(t, host.Dimensions)
	assert.Len(t, host.Dimensions, 2) // desktop (inherited) + bypass (injected)

	// Verify inherited dimension
	desktop, exists := host.Dimensions["desktop"]
	assert.True(t, exists)
	assert.Equal(t, 1, desktop.ID)
	assert.Equal(t, 1920, desktop.Width)
}
...
`
	...
	host := hosts[0]
	assert.Len(t, host.Dimensions, 2) // desktop + bypass (injected)
}
