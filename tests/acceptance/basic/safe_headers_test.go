...
var _ = Describe("Safe Headers - Response Header Filtering", Serial, func() {
	...
	// - X-Custom-Header
	//
	// Test non-safe headers (should be filtered):
	// - X-Secret-Header
	// - Set-Cookie
	// - X-Internal-Debug
	...
	Context("Rendered Response Headers", func() {
		...
		It("should include safe headers with correct values in rendered response", func() {
			...
			By("Verifying safe headers are present with correct values")
			...
			By("Verifying non-safe headers are filtered out")
	...
	Context("Edge Cases", func() {
		...
		It("should handle empty safe headers configuration gracefully", func() {
			...
			By("Verifying non-safe headers are filtered")
			Expect(resp.Headers.Get("X-Secret-Header")).To(BeEmpty(), "X-Secret-Header should be filtered")
		})
